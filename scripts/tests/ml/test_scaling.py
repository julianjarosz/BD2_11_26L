...
class TestScaleDataFit:
    def test_scaled_features_in_0_1_range(self) -> None:
        ...
        df = _make_df(rng)
        scaled, _, _ = scale_data(df, FEATURE_COLS, TARGET_COLS, MinMaxScaler(), MinMaxScaler(), fit=True)
    ...
    def test_returns_fitted_scalers(self) -> None:
        ...
        _, returned_X, returned_y = scale_data(df, FEATURE_COLS, TARGET_COLS, scaler_X, scaler_y, fit=True)
...
class TestScaleDataTransformOnly:
    def test_transform_uses_existing_scaler(self) -> None:
        ...
        scaler_X = MinMaxScaler()
        scaler_y = MinMaxScaler()
        _, scaler_X, scaler_y = scale_data(df_train, FEATURE_COLS, TARGET_COLS, scaler_X, scaler_y, fit=True)

        scaled_test, _, _ = scale_data(df_test, FEATURE_COLS, TARGET_COLS, scaler_X, scaler_y, fit=False)
