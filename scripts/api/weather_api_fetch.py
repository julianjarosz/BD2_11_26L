...
class OpenWeatherFetchManager:
    ...
    def _fetch_city_data_from_all_apis(
        self, city: str, **kwargs: dict[str, Any]
    ) -> dict[str, dict]:
    ...
    def _merge_data_from_api(self, city_data_from_apis: dict[str, dict]) -> OpenWeatherData:
        raw_data_buffer_fields: list[str] = (
            ...
        )  # This function needs to know what fields it will be looking
    ...
    def _fetch_data(self, city: str, **kwargs: dict[str, Any]) -> OpenWeatherData:
        ...
        return OpenWeatherData.from_api_response(
    ...
    async def fetch_cities(self, cities: Iterable[str], **kwargs: Any) -> list[OpenWeatherData]:
