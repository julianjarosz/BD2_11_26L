    ...
    with psycopg.connect(DB_URL) as conn:
        with conn.cursor() as cur:
            ...
            for i in range(60):
                ...
                cur.execute("""
                    INSERT INTO dw.dim_date (date_key, full_date, day, month, month_name, quarter, year, day_of_the_week, day_name, is_weekend)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT DO NOTHING;
                """, (
                    date_key, current_date.date(), current_date.day, current_date.month, 
                    current_date.strftime("%B"), (current_date.month - 1) // 3 + 1, current_date.year,
                    current_date.isoweekday(), current_date.strftime("%A"), current_date.weekday() >= 5
                ))
                ...
                cur.execute("""
                    INSERT INTO dw.dim_time (time_key, full_time, hour, minute, second, part_of_the_day)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    ON CONFLICT DO NOTHING;
                """, (time_key, current_date.time(), current_date.hour, current_date.minute, current_date.second, part))
                ...
                """, (
                    loc_key, date_key, time_key, current_date,
                    random.randint(1, 5), random.uniform(100, 500), random.uniform(0, 10), random.uniform(5, 50),
                    random.uniform(20, 100), random.uniform(1, 20), random.uniform(5, 80), random.uniform(10, 100), random.uniform(0, 5)
                ))
                
                # Insert fact_current_weather
                cur.execute("""
                    INSERT INTO dw.fact_current_weather (
                        location_key, observed_date_key, observed_time_key, weather_condition_key, observed_at,
                ...
                """, (
                    loc_key, date_key, time_key, cond_key, current_date,
                    random.uniform(-5, 30), random.randint(30, 90), random.uniform(0, 15)
                ))
